...
_ABL_DUMP_INC = """\
/* pyoe: incremental dump (delta) comparing DICTDB vs DICTDB2.
   DICTDB  = the database to be altered  (first -db arg)
   DICTDB2 = the database with the desired schema (second -db arg)
   Output file receives the .df statements needed to bring DICTDB in line
   with DICTDB2. */
/* SESSION:PARAMETER = tables|outfile|codepage */
DEFINE VARIABLE cTables   AS CHARACTER NO-UNDO INITIAL "ALL".
DEFINE VARIABLE cOutFile  AS CHARACTER NO-UNDO.
...
_ABL_LOAD_DF = """\
/* pyoe: load a .df schema file into the connected database silently */
